	...
	public function display($file, $template, $cacheId = null, $compileId = null)
	{
		...
		if (Module::isInstalled('froggytoolbarv2'))
		{
			...
			if ((int)$cookie_fc->id_employee > 0)
			{
				...
				$result = '<section class="froggytoolbar-debug-module-block"><span class="froggytoolbar-debug-module-name"><strong>'.$this->displayName.'</strong>: Template <span title="'.dirname($file).'/'.$template.'">'.$template.'</span> - '.($configuration_link != '' ? ' <a target="_blank" href="'._PS_BASE_URL_.__PS_BASE_URI__.$configuration_link.'">Configuration</a>' : '').'</span>'.$result.'</section>';
			}
		}
